...
*/
	...
	public void insertCanal(Path path){
		CanalReader canal = new CanalReader();
		List<Canal> listaRegistros = canal.readCanal(path);
	...
	public void insertProduto(Path path) {
		...
		try {
			...
			LOGGER.debug(Messages.ERRO_ID_FAMILIA);
				...
				for (Produto imp : produtos) {
					for (Phase expPh : dao.exportarPhase()) {
						if (imp.getSku() == expPh.getSkuNew() || imp.getSku() == expPh.getSkuOld()) {
							regPhase++;
							set.add(imp.getSku() + ", " + imp.getDescricao() + ", " + imp.getCodFamilia() + "\n");
						}
					}
				}
				if (regPhase > 0) {
...
*/
	public void insertPhase(Path path) {
		...
		PhaseReader leitor = new PhaseReader();
		List<Phase> listaRegistros = leitor.lerCsvPhase(path);
		ProdutoDAO registro = new ProdutoDAO();
		registro.insertSkuPhase(listaRegistros);
	}
